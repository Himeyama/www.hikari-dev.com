---
title: "I Built “proxa,” an LLM API Translation Proxy That Connects Claude Code, Codex, and Gemini"
authors: hikari
tags: [LLM]
---
...
**proxa** is the tool that solves these problems.

GitHub: https://github.com/Himeyama/proxa

## What is proxa?

proxa is a proxy server that converts multiple LLM APIs back and forth.
...
One use case for proxa is connecting Claude Code to a local LLM.
...
Then simply change Claude Code’s connection target to proxa, and you can use a local LLM from Claude Code.
...
proxa implements the `/v1/responses` endpoint and supports not only HTTP but also WebSocket.

That makes it easy to connect with Responses API-based clients such as the Codex CLI.

## Useful Logging Features

proxa includes a log display feature.
...
proxa, on the other hand, is structured like this:
...
proxa, on the other hand, accepts the Anthropic Messages API directly.
...
This is a very major feature of proxa.
...
proxa sits in the middle and absorbs the differences in API specifications.
...
If LiteLLM is a “unified API for developers,” proxa could be described as a “client-compatible proxy.”